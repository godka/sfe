...
Public Sub CreateBitmap_4(ByteArray() As Byte, PicWidth As Long, PicHeight As Long, Orientation As Integer, Optional Colorused As Long = 0)
    ...
    With bm4.bmiHeader
        ...
        .biWidth = PicWidth
                If Orientation = 0 Then
                .biHeight = PicHeight                    'Bitmap 高度, bitmap 置顶
                Else
                .biHeight = -PicHeight
                End If
        .biPlanes = 1
...
Public Sub DrawBitmap(PicWidth As Long, PicHeight As Long, PicObject As Object, Scalierung As Boolean, Optional X As Long = 0, Optional Y As Long = 0, Optional DrawToBG As Boolean = False)
    ...
    Select Case Scalierung
        Case True
            ergabe = PixelToTwips(PicWidth, PicHeight)
            If DrawToBG = False Then
                PicObject.Height = ergabe.Height
                PicObject.Width = ergabe.Width
            End If
        Case False
    End Select
    If DrawToBG = False Then
        If PicObject.Height <> PicObject.ScaleHeight Then 'with Boarders
        ergabe = Twipstopixel(PicObject.Width, PicObject.Height)
        realheight = ergabe.Height
        realwidth = ergabe.Width
    ...
    Select Case ThreeToOrToFour
        ...
        CopyMemory Bits(0), ByteArray(0), UBound(ByteArray)
            With bm24.bmiHeader
                ...
                .biWidth = PicWidth                  'Bitmap 宽度
                If Orientation = 0 Then
                .biHeight = PicHeight                    'Bitmap 高度, bitmap 置顶
                Else
                .biHeight = -PicHeight
                End If
                .biBitCount = 32                        '32 bit
                .biPlanes = 1                           'Single plane
                .biCompression = BI_RGB                 '不压缩
                .biSizeImage = 0                        '默认
                .biXPelsPerMeter = 0                    '默认
                .biYPelsPerMeter = 0                    '默认
                .biClrUsed = 0                          '默认
                .biClrImportant = 0                     '默认
            End With
...
Public Function InitColorTable_Grey(BitDepth As Integer, Optional To8Bit As Boolean = False) As Byte()
 ...
 CopyMemory ByVal VarPtr(bm8.bmiColors(0).rgbBlue), ByVal VarPtr(Tbl(0).rgbBlue), 1024
  End Select
  ReDim ergabe(((UBound(Table3) + 1) * 3) - 1)
  CopyMemory ergabe(0), ByVal VarPtr(Table3(0).Red), ((UBound(Table3) + 1) * 3)
InitColorTable_Grey = ergabe
End Function
